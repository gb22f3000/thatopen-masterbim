...
export const todoTool = (state: TodoUIState) => {
  ...
  const taskInput = document.createElement('bim-text-input') as BUI.TextInput
      ...
      <dialog>
        <bim-panel style="width: 20rem;">
          <bim-panel-section label="To-Do" fixed>
            <bim-label>Create a to-do linked to the current 3D selection</bim-label>
            ${nameInput}
            ${taskInput}
            ${priorityInput}
            <bim-button
              ...
              @click=${() => {
                ...
                void todoCreator.addTodo(todoValue)
                nameInput.value = ''
                taskInput.value = ''
                todoModal.close()
              }}
